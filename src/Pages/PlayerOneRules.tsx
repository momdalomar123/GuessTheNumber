  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-15 h-screen justify-start items-center mt-10 ">
        <p className="text-4xl text-white">Player 1 Chooses:</p>
        ...
        <div className="flex flex-col gap-1 ">
          ...
          <input
            ref={attemptInput}
            className="bg-zinc-600 px-4 pt-1 pb-1 rounded-md outline-0 text-2xl text-white"
            placeholder="Enter The Attempts"
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <div className="flex flex-col gap-0.5 justify-center items-center">
          {(noInputMessage && (
            <WrongNoInputMessage message="Please Enter All The Fields" />
          )) ||
            (wrongInputMessage && (
              <WrongNoInputMessage message="Please Enter Valid Inputs" />
            ))}
          <button
            className="text-2xl w-35 bg-gray-400 px-5 rounded-md cursor-pointer transition-all hover:bg-gray-500 hover:scale-105 active:bg-gray-300"
            onClick={checkInputAndProceed}
          >
            Proceed
          </button>
        </div>
